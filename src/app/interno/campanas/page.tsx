...
import { createCampaign } from "@/app/interno/actions";
...
function maybeEuros(value: number | null) {
  return value === null ? "Sin dato" : formatEuros(value);
}
  ...
  return (
    <div className="grid gap-4">
      <header className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          <p className="text-sm font-semibold text-primary">Fase 1 · Campaign CRM</p>
          <h1 className="mt-1 text-2xl font-semibold tracking-normal md:text-3xl">
            Campañas comerciales
          </h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-ink-soft">
            Cada campaña agrupa landing, leads, pipeline, tareas y métricas. Aquí empieza el
            sistema central antes de crear pantallas con IA.
          </p>
        </div>
        <Link
          href="/interno/leads"
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-lg border border-border bg-surface p-4">
          ...
          <p className="mt-1 text-xs font-semibold text-primary">{summary.campaigns} campañas totales</p>
        </article>
        <article className="rounded-lg border border-border bg-surface p-4">
          <p className="inline-flex items-center gap-2 text-sm font-semibold text-ink-soft">
          ...
          <p className="mt-1 text-xs font-semibold text-primary">{summary.leads} leads totales</p>
        </article>
        <article className="rounded-lg border border-border bg-surface p-4">
          <p className="inline-flex items-center gap-2 text-sm font-semibold text-ink-soft">
          ...
          <p className="mt-1 text-xs font-semibold text-primary">valor potencial estimado</p>
        </article>
        <article className="rounded-lg border border-border bg-surface p-4">
          <p className="inline-flex items-center gap-2 text-sm font-semibold text-ink-soft">
          ...
          <p className="mt-2 text-xl font-semibold">Leads y tareas</p>
          <p className="mt-2 text-xs font-semibold text-primary">priorizar llamadas antes de automatizar</p>
        </article>
      </section>
              ...
              >
                <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 text-sm leading-6 text-ink-soft">
                      {campaign.offer || "Sin oferta definida todavía."}
                    </p>
                    {campaign.goal ? (
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4 lg:w-[460px]">
                    ...
                    <div className="rounded-md bg-background p-3">
                      <p className="text-xs font-semibold text-ink-soft">Score medio</p>
                      <p className="mt-1 text-lg font-semibold">{avgScore}</p>
                    </div>
                    <div className="rounded-md bg-background p-3">
                      <p className="text-xs font-semibold text-ink-soft">Alta prioridad</p>
                      <p className="mt-1 text-lg font-semibold">{highPriority}</p>
                    </div>
                    <div className="rounded-md bg-background p-3">
                      <p className="text-xs font-semibold text-ink-soft">Ganadas</p>
                ...
                <div className="mt-4 grid gap-3 border-t border-border pt-4 lg:grid-cols-[1fr_auto] lg:items-center">
                  <div className="flex flex-wrap gap-2 text-xs font-semibold text-ink-soft">
                    ...
                    <span className="rounded-md bg-background px-2 py-1">Presupuesto: {maybeEuros(campaign.budgetCents)}</span>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Link
                      href={`/interno/leads?campaign=${campaign.id}`}
                    ...
                    <Link
                      href={`/interno/deals?campaign=${campaign.id}`}
                      className="inline-flex h-9 items-center justify-center rounded-md border border-border px-3 text-sm font-semibold hover:bg-muted"
                    >
                      Oportunidades
                    </Link>
        ...
        <aside className="rounded-lg border border-border bg-surface p-4 xl:sticky xl:top-4 xl:self-start">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-ink-soft">Crea el contenedor comercial antes de la landing.</p>
            </div>
          </div>
          ...
          <form action={createCampaign} className="mt-4 grid gap-3 text-sm">
            <label className="grid gap-1 font-semibold">
              Nombre
              <input className="h-10 rounded-md border border-border bg-white px-3 outline-none focus:border-primary" name="name" placeholder="Clínica Dental Sin Fugas" required />
            </label>
            <label className="grid gap-1 font-semibold">
              Vertical
              <input className="h-10 rounded-md border border-border bg-white px-3 outline-none focus:border-primary" name="vertical" placeholder="Clínicas dentales" required />
            </label>
            <label className="grid gap-1 font-semibold">
              Oferta
              <input className="h-10 rounded-md border border-border bg-white px-3 outline-none focus:border-primary" name="offer" placeholder="Auditoría gratuita, demo, piloto..." />
            </label>
            <label className="grid gap-1 font-semibold">
              Ruta de landing
              <input className="h-10 rounded-md border border-border bg-white px-3 outline-none focus:border-primary" name="landingPath" placeholder="/auditoria-fugas-dental" />
            </label>
            ...
            <label className="grid gap-1 font-semibold">
              Objetivo
              <textarea className="min-h-24 rounded-md border border-border bg-white px-3 py-2 outline-none focus:border-primary" name="goal" placeholder="Qué queremos validar, vender o aprender con esta campaña." />
            </label>
