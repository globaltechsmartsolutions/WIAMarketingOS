...
import { completeTask, updateDealStage } from "@/app/internal/actions";
...
const stageTabs = [
  ...
  { label: "Qualification", value: "cualificacion" },
  { label: "Demo", value: "demo" },
  { label: "Proposal", value: "propuesta" },
...
const noticeMessages: Record<string, string> = {
  deal_stage_updated: "Opportunity stage updated.",
  lost_reason_missing: "Add a reason before marking an opportunity as lost.",
...
function buildReturnTo(stage: string, q: string, campaign: string) {
  ...
  return `/internal/deals${query ? `?${query}` : ""}`;
}
  ...
  return (
    <div className="grid gap-4">
      <header className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          <p className="text-sm font-semibold text-primary">Commercial Pipeline</p>
          <h1 className="mt-1 text-2xl font-semibold tracking-normal md:text-3xl">
            Opportunities
          </h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-ink-soft">
            Deals connected to campaigns and leads. Use them to see what can close, where deals
            get stuck and which commercial tasks need to be executed.
          </p>
        </div>
        <Link
          href="/internal/leads"
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-lg border border-border bg-surface p-4">
          ...
          <p className="mt-1 text-xs font-semibold text-primary">{openDeals} open opportunities</p>
        </article>
        <article className="rounded-lg border border-border bg-surface p-4">
          <p className="inline-flex items-center gap-2 text-sm font-semibold text-ink-soft">
          ...
          <p className="mt-1 text-xs font-semibold text-primary">{averageProbability}% average probability</p>
        </article>
        <article className="rounded-lg border border-border bg-surface p-4">
          <p className="inline-flex items-center gap-2 text-sm font-semibold text-ink-soft">
          ...
          <p className="mt-1 text-xs font-semibold text-primary">{totalDeals} total opportunities</p>
        </article>
        <article className="rounded-lg border border-border bg-surface p-4">
          <p className="inline-flex items-center gap-2 text-sm font-semibold text-ink-soft">
          ...
          <p className="mt-1 text-xs font-semibold text-primary">based on applied filters</p>
        </article>
      </section>

      <section className="grid gap-3 rounded-lg border border-border bg-surface p-4 xl:grid-cols-[1fr_auto] xl:items-end">
        <form action="/internal/deals" className="grid gap-3 md:grid-cols-[1fr_240px_auto]">
          ...
          <label className="flex h-10 min-w-0 items-center gap-2 rounded-md border border-border bg-white px-3 text-sm text-ink-soft">
            ...
            <input
              ...
              defaultValue={q}
              name="q"
              placeholder="Search opportunity, company or reason"
            />
          </label>
          <select
      ...
      <section className="grid gap-3">
        {deals.length > 0 ? (
            ...
            <article id={`deal-${deal.id}`} key={deal.id} className="rounded-lg border border-border bg-surface p-4">
              <div className="grid gap-4 xl:grid-cols-[1fr_300px_280px]">
                <div className="min-w-0">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-ink-soft">
                    <span className="rounded-md bg-background px-2 py-1">
                      Value: {formatEuros(deal.valueCents ?? 0)}
                    </span>
                    <span className="rounded-md bg-background px-2 py-1">
                      Expected close: {deal.expectedCloseAt ? formatDateOnly(deal.expectedCloseAt) : "no date"}
                    </span>
                    {deal.campaign ? (
                      <Link className="rounded-md bg-background px-2 py-1 hover:bg-muted" href={`/internal/campaigns#campaign-${deal.campaign.id}`}>
                        {deal.campaign.name}
                      </Link>
                    ) : null}
                    {deal.lead ? (
                      <Link className="rounded-md bg-background px-2 py-1 hover:bg-muted" href={`/internal/leads#lead-${deal.lead.id}`}>
                ...
                <div className="grid content-start gap-3 rounded-md bg-background p-3 text-sm">
                  <p className="font-semibold">Move opportunity</p>
                  <form action={updateDealStage} className="grid gap-2">
                    ...
                    <select className="h-9 rounded-md border border-border bg-white px-3" defaultValue={deal.stage} name="stage">
                      <option value="cualificacion">Qualification</option>
                      <option value="demo">Demo</option>
                      <option value="propuesta">Proposal</option>
                      <option value="piloto">Pilot</option>
                      <option value="ganado">Won</option>
                      <option value="perdido">Lost</option>
                    </select>
                    <input className="h-9 rounded-md border border-border bg-white px-3" name="lostReason" placeholder="Reason if lost" />
                  ...
                  <div className="border-t border-border pt-3">
                    <p className="text-xs font-semibold text-ink-soft">Last movement</p>
                    {deal.activities.length > 0 ? (
                      ...
                      <p className="mt-2 text-ink-soft">No activity yet.</p>
                    )}
                  </div>
                </div>
          ...
          <div className="rounded-lg border border-dashed border-border bg-surface p-8 text-center">
            <h2 className="font-semibold">No opportunities in this view</h2>
            <p className="mt-2 text-sm text-ink-soft">
              Change the filter or review leads to create new opportunities.
            </p>
            <Link
              href="/internal/leads"
